...
public class ReportService implements IReportService {
    ...
    public Object getStopHourComparation(Map<String, Object> info) throws Exception {
        ...
        String sql = " select month(rep.start_date) as month, " +
                     ...
                     "        SUM(select sum(IFNULL(rre.work_hour,0)) " +
                     "           from r_repair rre " +
                     "          where rep.plan_sid = rre.plan_sid "+
                     "            and DATE_FORMAT(rep.start_date,'%Y%m') = DATE_FORMAT(rp.finish_date,'%Y%m') " +
                     "        ) as actual_stop_hour "+
                     "   from r_repair rep " +
                     "   left join r_plan rp " +
                     ...
                     "  group by DATE_FORMAT(rep.start_date,'%m') " +
                     "    -${tenantsid} ";
